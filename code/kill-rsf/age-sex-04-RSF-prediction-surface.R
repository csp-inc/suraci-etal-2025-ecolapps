...
# ------------------------------------------------------------------------------
# Adult female
ad_fem <- subset(selected_df, age_sex =="Female_adult")
load(file = "age-sex-class-RSFs/model-outputs/top-final-RSF-ad-fem.rda", verbose = TRUE) # load RSFs
ad_fem_covs = gridCovs(covs_stack, aoi, ad_fem) 
ad_fem_pred <- gridPredict(ad_fem_covs, age_sex_models_final_top, scale = "exp") 
ad_fem_rast <- gridRast(ad_fem_pred, filename = "age-sex-class-RSFs/model-outputs/predictions/ad-fem-exp-aoi-30.tif") 

# Adult male
ad_male <- subset(selected_df, age_sex =="Male_adult")
load(file = "age-sex-class-RSFs/model-outputs/top-final-RSF-ad-male.rda", verbose = TRUE) # load RSFs
ad_male_covs = gridCovs(covs_stack, aoi, ad_male) 
ad_male_pred <- gridPredict(ad_male_covs, age_sex_models_final_top, scale = "exp") 
ad_male_rast <- gridRast(ad_male_pred, filename = "age-sex-class-RSFs/model-outputs/predictions/ad-male-exp-aoi-30.tif") 

# Disperser female
disp_fem <- subset(selected_df, age_sex =="Female_disperser")
load(file = "age-sex-class-RSFs/model-outputs/top-final-RSF-disp-fem.rda", verbose = TRUE) # load RSFs
disp_fem_covs = gridCovs(covs_stack, aoi, disp_fem) 
disp_fem_pred <- gridPredict(disp_fem_covs, age_sex_models_final_top, scale = "exp") 
disp_fem_rast <- gridRast(disp_fem_pred, filename = "age-sex-class-RSFs/model-outputs/predictions/disp-fem-exp-aoi-30.tif")

# Disperser male
disp_male <- subset(selected_df, age_sex =="Male_disperser")
load(file = "age-sex-class-RSFs/model-outputs/top-final-RSF-disp-male.rda", verbose = TRUE) # load RSFs
disp_male_covs = gridCovs(covs_stack, aoi, disp_male) 
disp_male_pred <- gridPredict(disp_male_covs, age_sex_models_final_top, scale = "exp") 
disp_male_rast <- gridRast(disp_male_pred, filename = "age-sex-class-RSFs/model-outputs/predictions/disp-male-exp-aoi-30.tif") 
