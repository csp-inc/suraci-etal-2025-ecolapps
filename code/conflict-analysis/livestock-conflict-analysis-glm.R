## ---------------------------
##
## Script name: livestock-conflict-analysis-glm.R
##
...
conflict_raw <- st_read("/Users/patrickfreeman-csp/Documents/GitHub/pred-services/00-data/conflict-data/cougage-ALL-conflict-2016to2022/cougar_ALL_conflict_2016to2022.shp") %>%
...
all_cats_ssf <- rast("00-data/movement-ssfs/all-cats-ssf-prediction-30m.tif")
names(all_cats_ssf) <- "all_cats_SSF"

ad_m_ssf <- rast("00-data/movement-ssfs/ad-male-ssf-prediction-30m.tif")
names(ad_m_ssf) <- "ad_m_SSF"

disp_m_ssf <- rast("00-data/movement-ssfs/disp-male-ssf-prediction-30m.tif")
names(disp_m_ssf) <- "disp_m_SSF"

disp_f_ssf <- rast("00-data/movement-ssfs/disp-fem-ssf-prediction-30m.tif")
names(disp_f_ssf) <- "disp_f_SSF"

ad_f_ssf <- rast("00-data/movement-ssfs/ad-fem-ssf-prediction-30m.tif")
names(ad_f_ssf) <- "ad_f_SSF"


all_cats_rsf <- rast("00-data/kill-site-rsfs/age-sex-pooled-exp-aoi-30.tif")
names(all_cats_rsf) <- "all_cats_RSF"

ad_m_rsf <- rast("00-data/kill-site-rsfs/ad-male-exp-aoi-30.tif")
names(ad_m_rsf) <- "ad_m_RSF"

disp_m_rsf <- rast("00-data/kill-site-rsfs/disp-male-exp-aoi-30.tif")
names(disp_m_rsf) <- "disp_m_RSF"

disp_f_rsf <- rast("00-data/kill-site-rsfs/disp-fem-exp-aoi-30.tif")
names(disp_f_rsf) <- "disp_f_RSF"

ad_f_rsf <- rast("00-data/kill-site-rsfs/ad-fem-exp-aoi-30.tif")
names(ad_f_rsf) <- "ad_f_RSF"


# Load sampling domain ----------------------------------------------------

sampling_domain <- st_read("00-data/sampling-domain/all_ag_VIIRS_dev_full_yr_comb_buffer3km_clipland.shp")
...
conflict_n <- nrow(livestock_conflict_tidy)
...
background_pts <- backgroundSample(mask=mask, n=10*conflict_n, p=vect(livestock_conflict_tidy), excludep=T, tryf=9) %>%
  as_tibble() %>%
  st_as_sf(coords=c(c("x", "y")), crs=crs(livestock_conflict_tidy )) %>%
  dplyr::mutate(point_type="background") %>%
  dplyr::relocate(point_type, .before=everything())

# Create final conflict-background dataset ------------------------------------------------------
all_conflict_bg <- bind_rows(livestock_conflict_tidy, background_pts) %>%
  st_buffer(., 100)
...
livestock_conflict_prepared_data <- left_join(ssf_data_extract, rsf_data_extract, by=c("id", "conflict", "ReportDate","month")) %>%
...
ad_m_livestock_conflict <- livestock_conflict_prepared_data %>% 
  dplyr::select(id, conflict, ReportDate, month, ad_m_SSF, ad_m_rsf) 

disp_m_livestock_conflict <- livestock_conflict_prepared_data %>% 
  dplyr::select(id, conflict, ReportDate, month, disp_m_SSF, disp_m_rsf)

ad_f_livestock_conflict <- livestock_conflict_prepared_data %>% 
  dplyr::select(id, conflict, ReportDate, month, ad_f_SSF, ad_f_rsf)

disp_f_livestock_conflict <- livestock_conflict_prepared_data %>% 
  dplyr::select(id, conflict, ReportDate, month, disp_f_SSF, disp_f_rsf)

allcats_livestock_conflict <- livestock_conflict_prepared_data %>%
...
ad_m_livestock_model <- glm(conflict ~ ad_m_SSF + ad_m_rsf, data=ad_m_livestock_conflict, family="binomial")
disp_m_livestock_model <- glm(conflict ~ disp_m_SSF + disp_m_rsf, data=disp_m_livestock_conflict, family="binomial")
ad_f_livestock_model <- glm(conflict ~ ad_f_SSF + ad_f_rsf, data=ad_f_livestock_conflict, family="binomial")
disp_f_livestock_model <- glm(conflict ~ disp_f_SSF + disp_f_rsf, data=disp_f_livestock_conflict, family="binomial")
all_cats_livestock_model <- glm(conflict ~ all_cats_ssf + all_cats_rsf, data= allcats_livestock_conflict, family="binomial")
...
ad_m_livestock_tidy_out <- tidy(ad_m_livestock_model, conf.int = TRUE, conf.level = 0.95, exponentiate=F) %>%
...
disp_m_livestock_tidy_out <- tidy(disp_m_livestock_model, conf.int = TRUE, conf.level = 0.95, exponentiate=F) %>%
  dplyr::select(term, estimate, std.error, statistic, p.value, conf.low, conf.high) %>% 
  dplyr::filter(term != "(Intercept)") %>%
  dplyr::mutate(varType = case_when(str_detect(term, "SSF") == T ~ "ssf",
                                    str_detect(term, "rsf") == T ~ "rsf")) %>% 
...
ad_f_livestock_tidy_out <- tidy(ad_f_livestock_model, conf.int = TRUE, conf.level = 0.95, exponentiate=F) %>%
  dplyr::select(term, estimate, std.error, statistic, p.value, conf.low, conf.high) %>% 
  dplyr::filter(term != "(Intercept)") %>%
  dplyr::mutate(varType = case_when(str_detect(term, "SSF") == T ~ "ssf",
                                    str_detect(term, "rsf") == T ~ "rsf")) %>%
...
disp_f_livestock_tidy_out <- tidy(disp_f_livestock_model, conf.int = TRUE, conf.level = 0.95, exponentiate=F) %>%
  dplyr::select(term, estimate, std.error, statistic, p.value, conf.low, conf.high) %>% 
  dplyr::filter(term != "(Intercept)") %>%
  dplyr::mutate(varType = case_when(str_detect(term, "SSF") == T ~ "ssf",
                                    str_detect(term, "rsf") == T ~ "rsf")) %>% 
...
allcats_livestock_tidy_out <- tidy(all_cats_livestock_model, conf.int = TRUE, conf.level = 0.95, exponentiate=F) %>%
  dplyr::select(term, estimate, std.error, statistic, p.value, conf.low, conf.high) %>% 
  dplyr::filter(term != "(Intercept)") %>%
  dplyr::mutate(varType = case_when(str_detect(term, "ssf") == T ~ "ssf",
                                    str_detect(term, "rsf") == T ~ "rsf")) %>% 
...
all_livestock_model_coefs <- bind_rows(list(ad_m_livestock_tidy_out, disp_m_livestock_tidy_out, ad_f_livestock_tidy_out, disp_f_livestock_tidy_out, allcats_livestock_tidy_out))

all_livestock_model_coefs$ASClass <- factor(all_livestock_model_coefs$ASClass, levels=rev(c("all_cats", "adult_female", "adult_male", "disp_female", "disp_male")), ordered=T)
...
all_livestock_model_coefs <- all_livestock_model_coefs %>%
  dplyr::mutate(
...
(livestock_dwplot <- 
    dwplot(arrange(all_livestock_model_coefs, -desc(ASClass)),
       ci = 0.95,
  ...
  scale_fill_manual(values=c(all_col, af_col, am_col, df_col, dm_col), 
                     ...
                     name="Age/Sex Class") + 
  scale_shape_manual(name  ="Predictor Type", 
                       breaks = c("rsf", "ssf"), 
                       values = c(21, 22),
                       labels=c("Kill site RSF", "Movement SSF")) + # breaks assign shapes
...
(livestock_conflict_coef_plot <- ggplot(data = all_livestock_model_coefs, 
       aes(x = ASClass, 
           y = estimate, 
           fill = ASClass, 
           shape=varType),
...
pdf(file = '00-out/livestock-only-conflict-coefs-20240306.pdf', height = 6, width = 6)
grid.arrange(livestock_conflict_coef_plot, nrow = 1)
dev.off()
